public class Morse{
  public static enum Symbol { DOT, DASH };
  ...
  public static Symbol[][][] toMorse(String text){
    ...
    Stream<Symbol[][]> morseStream = wordStream.map(Morse::wordToMorse);
    return morseStream.toArray(Symbol[][][]::new);
  }

  private static Symbol[][] wordToMorse(String word){
    Symbol[][] morseWord = new Symbol[word.length()][];
    for(int i = 0 ; i < word.length() ; i++){
      morseWord[i] = toMorse(word.charAt(i));
    }
    return morseWord;
  }
}
